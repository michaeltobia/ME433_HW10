...
int len = 0, i = 100; // init i at 100 to prevent write when device is reset
int startTime = 0; // to remember the loop time
char whoami_check = 0;  // IMU whoami check
...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            // Read IMU at 100Hz and convert data into printable form
            I2C_read_multiple(SLAVE_ADDR, 0x20, data_store, 14); // Read IMU
            for(j = 0;j<7;j++){ // Convert IMU data
                data_conv[j] = (data_store[j*2+1] << 8) | data_store[j*2];
            }
            
            // flip whoami LED if IMU is still responding
            whoami_check = getReg(0x0F);
            if(whoami_check == 0x69){
                LATBINV = 0x100;
            }
            
            // print indexed IMU data as:
            //        "index, accZ, MAF, IIR, FIR"
            len = sprintf(dataOut, "%d %d %d %d %d\r\n", 
                    i, data_conv[6], maf(data_conv[6]), fir(data_conv[6]), iir(data_conv[6]));
